...
/// Gallery for any user (friend profile view) — only public meals
final userGalleryProvider = FutureProvider.family<List<GalleryItem>, ({String userId, DateTime month})>((ref, params) async {
  final supabase = ref.watch(supabaseProvider);

  final monthStart = DateTime(params.month.year, params.month.month, 1);
  final monthEnd = DateTime(params.month.year, params.month.month + 1, 0);
  final monthStartStr = monthStart.toIso8601String().split('T')[0];
  final monthEndStr = monthEnd.toIso8601String().split('T')[0];
